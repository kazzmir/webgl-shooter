...
func (enemy *NormalEnemy) Hit(bullet *Bullet) {
	...
	if enemy.hasElementStrength(bullet.ElementType) {
		damage *= 0.9
	}
	if enemy.hasElementWeakness(bullet.ElementType) {
		damage *= 1.1
	}
	enemy.Damage(damage)
}

func (enemy *NormalEnemy) hasElementStrength(element ElementType) bool {
	for _, strength := range enemy.Strengths {
		if strength == element {
			return true
		}
	}
...
func (enemy *NormalEnemy) hasElementWeakness(element ElementType) bool {
...
func MakeEnemy1(x float64, y float64, rawImage image.Image, image *ebiten.Image, move Movement, difficulty float64, strengths []ElementType, weaknesses []ElementType) (Enemy, error) {
	return &NormalEnemy{
		Kind:       "enemy1",
		x:          x,
		y:          y,
		...
		Strengths:  append([]ElementType(nil), strengths...),
		Weaknesses: append([]ElementType(nil), weaknesses...),
	}, nil
}

func MakeEnemy2(x float64, y float64, rawImage image.Image, pic *ebiten.Image, move Movement, difficulty float64, strengths []ElementType, weaknesses []ElementType) (Enemy, error) {
	return &NormalEnemy{
		Kind:       "enemy2",
		x:          x,
		y:          y,
