...
func ResizeBy(img image.Image, factor float64, interp InterpolationFunction) image.Image {
    return Resize(uint(float64(img.Bounds().Dx()) * factor), uint(float64(img.Bounds().Dy()) * factor), img, interp)
}
